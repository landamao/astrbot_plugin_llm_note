...
### 新增
- **全局共享笔记配置**：新增 `全局共享笔记` 布尔配置项，开启后笔记注入时遍历所有群组的笔记数据，非当前群的笔记标注 `[来自其他会话，仅供参考]` 前缀，当前群global正常注入，其他群global不注入，写入仍以原群组形式
- `Note.get_all_data()` 方法：返回所有群组的全部笔记数据，格式 `{group_id: {user_id: [notes]}}`
...
### 修复
- **恢复笔记校验报错信息错误**：`/恢复笔记` 校验 `id`/`type` 字段类型时，报错信息错误地打印了 `data` 的类型而非实际出问题的字段类型（`type(data)` → `type(i)`）
